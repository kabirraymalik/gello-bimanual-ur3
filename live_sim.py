...
#gello info
motorAngles = []
offsets = [-1.0334554334554336, 0.9896214896214897, -0.5402930402930404, 1.49995115995116, 0.00012210012210012692, -2.056166056166056]
...
def controller(model, data):
  ...
  for motor_id in range(len(motor_positions)-1):
    if joint_orientations[motor_id] == -1:
      ...
      data.qpos[motor_id] = motor_positions[motor_id] + offsets[motor_id]
  
  #right hand
  data.qpos[6] = ability_macro(dmR.get_position(7))
  data.qpos[7] = ability_macro(dmR.get_position(7))
  data.qpos[8] = ability_macro(dmR.get_position(7))
  data.qpos[9] = ability_macro(dmR.get_position(7))
  data.qpos[10] = ability_macro(dmR.get_position(7))
  data.qpos[11] = ability_macro(dmR.get_position(7))
  data.qpos[12] = ability_macro(dmR.get_position(7))
  data.qpos[13] = ability_macro(dmR.get_position(7))
  data.qpos[14] = ability_macro(dmR.get_position(7))
  data.qpos[15] = ability_macro(dmR.get_position(7))
